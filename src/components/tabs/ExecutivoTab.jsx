...
export default function ExecutivoTab() {
  ...
  const { data: honorarios, loading: loadingFinanceiro } = useSupabaseTable("honorarios", { select: "valor, status" });
  ...
  const financeiroResumo = [
    { nome: "Total", valor: totalHonorarios },
    { nome: "Recebido", valor: recebido },
    { nome: "A receber", valor: aReceber },
  ];
  ...
  return (
    <div>
      ...
      <Card className="mb-6">
        <p className="text-sm font-semibold mb-4" style={{ color: COLORS.ink }}>Financeiro — total, recebido e a receber</p>
        {!loadingFinanceiro && totalHonorarios === 0 ? (
          <p className="text-sm" style={{ color: COLORS.slate }}>Sem cobranças cadastradas ainda.</p>
        ) : (
          <div style={{ width: "100%", height: 220 }}>
            <ResponsiveContainer>
              <BarChart data={financeiroResumo} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
                ...
                <Bar dataKey="valor" radius={[4, 4, 0, 0]} fill={COLORS.brass} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
