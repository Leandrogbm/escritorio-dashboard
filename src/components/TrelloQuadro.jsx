  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-2">
      {listas.map((lista) => {
        ...
        return (
          <div key={lista.id} className="shrink-0" style={{ width: 280 }}>
            <Card>
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold" style={{ color: COLORS.ink }}>{lista.name}</p>
                <span className="text-xs" style={{ color: COLORS.slate }}>{doLista.length}</span>
              </div>
              <div
              ...
              >
                {doLista.map((c) => (
                  ...
                  >
                    ...
                    {c.desc && <p className="text-xs mt-1 whitespace-pre-wrap" style={{ color: COLORS.slate }}>{c.desc}</p>}
                  </div>
                ))}
              </div>
              {novaTarefaEm === lista.id ? (
                <form onSubmit={(e) => { e.preventDefault(); criarCard(lista.id); }} className="flex flex-col gap-1.5 mt-2">
                  <input autoFocus value={novoTitulo} onChange={(e) => setNovoTitulo(e.target.value)} placeholder="Título..." className="px-2 py-1.5 rounded text-sm" style={{ border: `1px solid ${COLORS.line}`, color: COLORS.ink }} />
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => { setNovaTarefaEm(null); setNovoTitulo(""); }} className="text-xs" style={{ color: COLORS.slate }}>Cancelar</button>
                  </div>
                </form>
              ) : (
                <button onClick={() => setNovaTarefaEm(lista.id)} className="flex items-center gap-1 mt-2 text-xs" style={{ color: COLORS.slate }}>
